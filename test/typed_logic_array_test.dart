...
void main() {
  group('TypedLogicArray', () {
    ...
    test('recursively flattens nested array dimensions with typed leaves', () {
      final nested = TypedLogicArray<LogicArray, LogicValue>(
        [2, 2],
        ({name}) => LogicArray([3, 2], 4, name: name),
      );
      final flattened = nested.flattenNestedDimensions<Logic, LogicValue>(
    ...
    test('flattens every nested array layer', () {
      final nested =
        ...
        [2],
        ({name}) => TypedLogicArray<LogicArray, LogicValue>(
          [3],
    ...
    test('flattens empty arrays and leaves without specialized clones', () {
      final empty = TypedLogicArray<Logic, LogicValue>(
        [2, 0],
        ({name}) => Logic(name: name, width: 4),
      );
      final emptyNested =
      ...
      final specialized =
          TypedLogicArray<_SpecialLogic, LogicValue>([2], _SpecialLogic.new);

      final flattenedEmpty = empty.flattenNestedDimensions<Logic, LogicValue>(
        valueCodec: LogicValueCodec.logicValue,
      );
      final flattenedNested =
          emptyNested.flattenNestedDimensions<Logic, LogicValue>(
        valueCodec: LogicValueCodec.logicValue,
      );
      final flattenedSpecialized =
          specialized.flattenNestedDimensions<_SpecialLogic, LogicValue>(
        valueCodec: LogicValueCodec.logicValue,
      );
