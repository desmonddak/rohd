...
void main() {
  ...
  test('every recorded SV position resolves to the right text', () async {
    ...
    expect(
      ...
      greaterThan(0.75),
      reason:
          '>=75% of SV positions should resolve. Failures:\n'
          '${failures.join('\n')}',
    );
  });

  test('at least one signal has multiple SV positions (declaration + '
      'assignment)', () async {
    final built = await _buildFilterBankFlc(packageRoot);
    final symbols = _collectAllSymbols(built.json);
    ...
    expect(
      ...
      isNotEmpty,
      reason:
          'FilterBank should contain signals/instances recorded at both '
