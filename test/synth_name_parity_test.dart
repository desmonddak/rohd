...
void main() {
  ...
  group('cross-synthesizer parity', () {
    ...
    test(
      ...
      () async {
        void runNetlist(_CollidingNames mod) => NetlistService(mod);
        void runSv(_CollidingNames mod) =>
            SvService(mod, register: false).synthOutput;
        ...
        expect(
            netlistOnly['secondDup'], isNot(equals(netlistOnly['firstDup'])));
      },
    );
    ...
    test(
      'colliding names stay stable when SV inlines one signal',
      () async {
        void runNetlist(_PartiallyInlineCollidingNames mod) =>
            NetlistService(mod);
        void runSv(_PartiallyInlineCollidingNames mod) =>
            SvService(mod, register: false).synthOutput;

        final netlistOnly = await _partialInlineCollisionNamesAfter([
          runNetlist,
        ]);
        await Simulator.reset();
