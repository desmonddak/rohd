...
Map<Logic, String> _collectNames(SynthModuleDefinition def) {
  ...
  for (final sl in [
    ...
    try {
      ...
      // Some synthesized signals are intentionally unnamed after pruning.
  ...
  for (final logic in svNames.keys) {
    if (scNames.containsKey(logic)) {
      expect(
        ...
        svNames[logic],
        reason:
            '$context: Name mismatch for Logic "${logic.name}" '
...
void main() {
  group('SystemC vs SystemVerilog naming consistency', () {
      ...
      for (final entry in lineMap.entries) {
        ...
        for (final lineCol in entry.value) {
          ...
          expect(
            ...
            contains(name),
            reason:
                'Line ${line + 1} should contain "$name".\n'
          ...
          if (colEnd <= lines[line].length) {
            expect(
              lines[line].substring(col, colEnd),
              equals(name),
    ...
    test(
      'scLineMap records multiple positions for re-assigned signals',
      () async {
        ...
        final synthBuilder = SynthBuilder(mod, SystemCSynthesizer());
        final results = synthBuilder.synthesisResults;
        final topResult = results.firstWhere(
          (r) => r.module == mod,
        ) as SystemCSynthesisResult;
        final text = topResult.toFileContents();
        final lineMap = topResult.scLineMap;
        final lines = text.split('\n');
        ...
        expect(
          ...
          reason:
              'Expected the declaration plus at least two assignment '
              'positions for `y`, got: $yPositions',
        );

        // Positions must be in textual (line-number) order.
        final lineNumbers = yPositions
            .map((p) => int.parse(p.split(':')[0]))
            .toList();
        ...
        // No duplicates.
        expect(
          yPositions.toSet().length,
          equals(yPositions.length),
        ...
        for (final p in yPositions) {
          ...
          expect(
            lines[line].substring(col, col + 1),
            equals('y'),
        ...
        expect(
          ...
          reason:
              'Expected at least two assignment LHS lines for `y`, '
              'got: $assignmentLines',
        );
      },
    );
