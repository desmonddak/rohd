...
- Added `LogicArrayOf<T>` for multidimensional arrays with fixed typed elements, including typed ports, traversal, cloning, shape operations, net support, and synthesis. Added shaped `LogicValueArray` and codec-backed `LogicValueArrayOf<T>` values with nested construction, packed `LogicValue` interoperability, and hardware-array assignment (<https://github.com/intel/rohd/pull/686>).
