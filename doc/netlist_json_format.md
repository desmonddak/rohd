...
| Consumed by | Nothing in this repository today — see below | `FlcService`/`FlcData` in the DevTools extension and schematic viewer (the actual cross-probe feature) |

In other words: if you enable `trace: true` on `NetlistSynthesizerConfiguration`,
you get `rohd.src_trace` attributes embedded in the netlist JSON as a
bonus, but the DevTools/schematic-viewer cross-probing UI you interact
with does not use them — it calls `TraceService.current?.flcJson` /
`.flcModuleJson()` instead, which is generated and served completely
independently of netlist synthesis. Treat `rohd.src_trace` as a
lightweight, embedded alternative for tools that only have the netlist
JSON and want a quick, unmerged pointer back to Dart source, not as part
of the FLC pipeline.
