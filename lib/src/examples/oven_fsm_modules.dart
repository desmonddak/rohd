...
//
// 2026 April
// Authors: Yao Jing Quek, Max Korbel
  ...
  OvenModule(Logic button, Logic reset, Logic clk)
    ...
    final states = [
      State<OvenState>(
        OvenState.standby,
        events: {
          Logic(name: 'button_start')..gets(
                button.eq(Const(Button.start.value, width: button.width)),
              ):
              OvenState.cooking,
        },
        actions: [led < LEDLight.blue.value, counterReset < 1, en < 0],
      ),
      State<OvenState>(
        OvenState.cooking,
        events: {
          Logic(name: 'button_pause')..gets(
                button.eq(Const(Button.pause.value, width: button.width)),
              ):
              OvenState.paused,
          Logic(name: 'counter_time_complete')..gets(counter.val.eq(4)):
              OvenState.completed,
        },
        actions: [led < LEDLight.yellow.value, counterReset < 0, en < 1],
      ),
      State<OvenState>(
        OvenState.paused,
        events: {
          Logic(name: 'button_resume')..gets(
                button.eq(Const(Button.resume.value, width: button.width)),
              ):
              OvenState.cooking,
        },
        actions: [led < LEDLight.red.value, counterReset < 0, en < 0],
      ),
      State<OvenState>(
        OvenState.completed,
        events: {
        ...
        actions: [led < LEDLight.green.value, counterReset < 1, en < 0],
      ),
    ];

    _oven = FiniteStateMachine<OvenState>(
      clk,
      reset,
      OvenState.standby,
      states,
